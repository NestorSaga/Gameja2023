...
public class NPCGenerator : MonoBehaviour
{
    ...
    public void GenerateNPC()
    {
       /* for (int i = 0; i<4;i++)
        ...
        int maskValue = 5;
        if(Random.Range(0,10) <= maskValue) transform.GetChild(1).GetComponent<Image>().sprite = RandomFromArray(masks);
        else transform.GetChild(1).GetComponent<Image>().sprite = empyMask; //Empty
